...
def getRel(data):
    ...
    querystring = "select CD_FREN_TRAN, SUM(QT_LIQUIDO) from APT_CARGAS where DT_ENTRADA like '%s' GROUP BY CD_FREN_TRAN ORDER BY CD_FREN_TRAN" % (
        data)
...
def getRelGeral(data):
    ...
    c = conn.cursor()
    querystring = x = "select SUM(QT_CANA_ENT),SUM(QT_CARGA_ENT),SUM(QT_CANA_ANL),SUM(VL_PZA),SUM(VL_PCC),SUM(VL_AGIO),SUM(VL_ATR) from PIMSCS.HISTUPNV1 where DT_REF like '%s' GROUP BY DT_REF" % (
        data)
    c.execute(querystring)
    dic = {}
    for row in c:
...
def getRelDensidade(data):
    ...
    conn = cx_Oracle.connect(user='PIMSCS', password='USJPIMS1', dsn=dsn_tns)
    c = conn.cursor()
    querystring = "select CD_FREN_TRAN, AVG(QT_LIQUIDO) from APT_CARGAS where DT_ENTRADA like '%s' GROUP BY CD_FREN_TRAN ORDER BY CD_FREN_TRAN" % (
        data)
    c.execute(querystring)
...
def getRelTMP(data):
    ...
    querystring = "select CD_FREN_TRAN,AVG(QT_HRQUEIMDES) from APT_CARGAS where DT_ENTRADA like '%s' and FG_CANA_CRUA = 'N' GROUP BY CD_FREN_TRAN" % (
        data)
    c.execute(querystring)
...
def getRelImpur(data):
    ...
    conn = cx_Oracle.connect(user='PIMSCS', password='USJPIMS1', dsn=dsn_tns)
    c = conn.cursor()
    querystring = "select CD_FREN_TRAN,AVG(QT_IMPUR_TERRA), AVG(QT_IMPUR_VEG) from APT_CARGAS where DT_ENTRADA like '%s' GROUP BY CD_FREN_TRAN" % (
        data)
    c.execute(querystring)
...
def getATR(data):
    ...
    c = conn.cursor()
    '''
    # c = connection('sja-hsdb03.grupocavalcanti.intranet', 1521, 'controle.usj.com.br', 'PIMSCS', 'USJPIMS1')
...
def getTC(data):
    ...
    querystring = "select SUM(QT_LIQUIDO/1000) from  APT_CARGAS where DT_ENTRADA like '%s' UNION select SUM(QT_LIQUIDO/1000) from APT_CARGAS where DT_ENTRADA like '%s'  and FG_ANALISE='S'" % (
    data, data)
...
def getRelPres(data):
    ...
    conn = cx_Oracle.connect(user='PIMSCS', password='USJPIMS1', dsn=dsn_tns)
    c = conn.cursor()
    querystring = "SELECT POSTOMETEO.DE_POSTO, CLIMAT.QT_LEITURA FROM CLIMAT JOIN POSTOMETEO ON CLIMAT.CD_POSTO = POSTOMETEO.CD_POSTO WHERE DT_OPERACAO like'%s'" % (
        data)
    c.execute(querystring)
...
def getRelBoletim(data):
    ...
    querystring = '''SELECT VAR_CODIGO,LAN_VALOR FROM PROLANCA WHERE LAN_DATA = '%s' AND VAR_CODIGO = 'CANPROPRIA' AND LAN_HORA = 'XXXX'
              ...
              UNION
              SELECT VAR_CODIGO,LAN_VALOR FROM PROLANCA WHERE LAN_DATA = '%s' AND VAR_CODIGO = 'PRODTOTHID' AND LAN_HORA = 'XXXX''' % (
